...
const CRYPTO_LIB: Record<string, CryptoDetail> = {
  'ecdsa-p256': {
    category: 'Signing',
...
function pick(...ids: string[]): CryptoDetail[] {
...
function longLifetime(ctx: ContextState) {
  return ctx.lifetime === '11-15' || ctx.lifetime === '16-20' || ctx.lifetime === '20+'
}

function wantsPqc(ctx: ContextState) {
  return ctx.pqc === 'plan' || ctx.pqc === 'ready'
}

function buildUseCase(objId: string, ctx: ContextState): UseCaseRecommendation {
  ...
  // Quantum vulnerability & migration
  ...
  // Threat level influence on confidence
  ...
  const priority: Priority = base.priorityWeight >= 8 ? 'HIGH' : base.priorityWeight >= 6 ? 'MEDIUM' : 'LOW'

  // Hardware impact
  const heavy = base.quantumVulnerable && hybridApplied
  const hardware: HardwareImpact[] = [
...
export function generateRecommendation(selections: Selections, ctx: ContextState): Recommendation {
...
export type CompareAlgo = {
  ...
  pqc: string
  automotiveReadiness: string
  migrationComplexity: string
}

export const COMPARE_ALGORITHMS: CompareAlgo[] = [
  {
    id: 'ecdsa-p256',
    name: 'ECDSA P-256',
