...
import type { Settings } from '@/lib/crypto/types'
...
type ToggleKey = 'backgroundEffects' | 'cursorEffects' | 'reduceAnimation' | 'highContrast'

const TOGGLES: { key: ToggleKey; title: string; desc: string; icon: typeof Sparkles }[] = [
  {
          ...
          return (
            ...
            >
              ...
              <Toggle
                checked={checked}
                label={t.title}
                onChange={(v) => setSettings({ [t.key]: v } as Partial<Settings>)}
              />
            </div>
          )
        })}
