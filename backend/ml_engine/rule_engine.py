...
def _resolve_use_case_entry(
    ...
    if isinstance(db, dict) and "use_cases" in db:
        db = db["use_cases"]

    if isinstance(db, list):
        for item in db:
            if isinstance(item, dict) and (item.get("useCase", "").lower() == use_case.lower() or item.get("use_case", "").lower() == use_case.lower()):
                return item
        return None
    ...
    synonyms = {
        "otaupdates": "OTA Update",
        "ota": "OTA Update",
        "otaupdate": "OTA Update",
        "firmwaresigning": "Digital Signatures",
        "signatures": "Digital Signatures",
        "firmware": "Digital Signatures",
        "v2x": "V2X Communication",
        "v2xcommunication": "V2X Communication",
        "can": "SecOC / In-Vehicle Communication",
        "secoc": "SecOC / In-Vehicle Communication",
        "invehicle": "SecOC / In-Vehicle Communication",
        "storage": "Data Encryption (At Rest & In Transit)",
        "dataencryption": "Data Encryption (At Rest & In Transit)",
        "encryption": "Data Encryption (At Rest & In Transit)",
        "boot": "Secure Boot",
        "secureboot": "Secure Boot",
        "keyexchange": "Key Management & Ephemeral Exchange",
        "lifecycle": "Key Management & Ephemeral Exchange",
        "keymanagement": "Key Management & Ephemeral Exchange",
        "provisioning": "Hardware Security Module (HSM) Provisioning",
        "hsm": "Hardware Security Module (HSM) Provisioning",
        "hsmprovisioning": "Hardware Security Module (HSM) Provisioning",
    }
